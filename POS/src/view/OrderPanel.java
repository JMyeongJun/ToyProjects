...
//side버튼 기능연결 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	public void actionPerformed(ActionEvent e) {												// dao에 클로즈 넣었습니다
		...
		case "삭제" : click_DelMenu(); break;
		case "+"    : click_PlsMenu(); break;
		case "-"    : click_MnsMenu(); break; 	
		case "결제" : click_Payment(); break;
		}
	} 
...
// bottom 패널 초기화면 --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	private void viewBasic() {
		...
		Vector<MenuVo> mvoList = mdao.getMenuListByCategoryId(vo.get(0).getCategoryID());															// vo 인덱스 0번째의 카테고리에 속한 메뉴들을 출력 
...
// top_bottom 패널, bottom 패널-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
	private void viewBottom() {
		...
		for(CategoryVo cvo : cdao.getCategoryList()) {																								// 카테고리 VO에 GetcategoryList 배열의 0번째 index부터 ~ 없을때까지 넣겠다. [1,커피] [2, 쥬스] [3, 에이드]
	         btnCategory.add(new JButton(cvo.getCategoryName()));																					// btncategory버튼에 cvo에서 name을 불러와서 넣겠다 (반복, 카테고리 dao의 getCategoryList가 없을때까지).[커피], [쥬스], [에이드]
	         System.out.println("버튼이름 추가: " + cvo.getCategoryName());
	      }
		
		  for(JButton btn : btnCategory) {																										    // 버튼 btn에 btn카테고리를 0번째 index부터 없을때까지 넣겠다. -> 버튼 3개 생성 [커피', [쥬스], [에이드], 버튼배열을 바로 패널위에 올릴 수 없기 때문에 한개한개 다시 만들어서 넣어준다.
		         btn.addActionListener(e -> {																										// btn이 클릭되면
		            ...
		            Vector<MenuVo> mvoList = mdao.getMenuListByCategoryId(cdao.getCategoryByName(e.getActionCommand()).getCategoryID());            // 카테고리 다오에서 버튼이름으로 카테고리를 찾은후 거기서 카테고리ID를 추출함 -> 메뉴다오에 카테고리 ID를 넣고 메뉴를 찾아서 VO리스트에 넣음 
...
// bottom -> menu 버튼 클릭 이벤트--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	class click_Menu implements ActionListener {
		...
		public void actionPerformed(ActionEvent e2) {
		  ...
		  //for (int i = 1; i<10; i++) {
		  // 	MenuVo  vo  = dao.getMenuById(i);													// ㄱ 현명하게 바꾸기
...
// 테이블 전달하기 -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	public static JTable getTable() {
		return tblOrder;																			// 벡터를 만들필요가 없는게 어차피 테이블이 모든 정보를 담고있었다...
	}
