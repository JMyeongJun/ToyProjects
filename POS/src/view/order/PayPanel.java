...
public class PayPanel extends BasicPanel {
	...
	private void initcomponents() {
		...
		gbc.weighty = 20.0;
		gbAdd(table, 0, 1, 1, 1);
		table.setBackground(colorBottom);
		bottom.add(table);
	...
	private void setBtnActions() {
		...
		btnUsePoint.addActionListener(e -> {
			if(member == null) {
				JOptionPane.showMessageDialog(null, "적립 먼저 해주세요.", "적립 확인", JOptionPane.WARNING_MESSAGE);
				return;
			}
			int usePoint = Integer
					.parseInt(JOptionPane.showInputDialog("사용가능 포인트 : " + Integer.toString(member.getPoint())));
	...
	private void pay(String paymentMethod) {
		...
		sdao.insertSales(paymentMethod, Integer.parseInt(labelUsePoint.getText()));

		JOptionPane.showMessageDialog(null, paymentMethod + " 되었습니다.");
		mf.changePanel(new OrderPanel(mf));
	}
